...
// Version: 6.0.3 - Cache Version 57 (BDM Analytics: per-cadence period selectors)
// ============================================

const CACHE_NAME = 'ebtracker-v57';
const STATIC_CACHE = 'ebtracker-static-v57';
const DYNAMIC_CACHE = 'ebtracker-dynamic-v57';
...
self.addEventListener('install', (event) => {
  console.log('🔧 Service Worker v57: Installing...');
  ...
  event.waitUntil(
    Promise.all([
      ...
      caches.open(STATIC_CACHE)
        .then((cache) => {
          console.log('📦 Service Worker v57: Caching static assets');
          return cache.addAll(STATIC_ASSETS);
        }),
      // Cache external scripts (Firebase SDK including Storage)
      caches.open(DYNAMIC_CACHE)
        .then((cache) => {
          console.log('📦 Service Worker v57: Caching Firebase SDK');
          return Promise.all(
    ...
    .then(() => {
      console.log('✅ Service Worker v57: All assets cached');
      return self.skipWaiting();
    })
    .catch((error) => {
      console.error('❌ Service Worker v57: Cache failed', error);
    })
  );
});
...
self.addEventListener('activate', (event) => {
  console.log('🚀 Service Worker v57: Activating...');
  ...
  event.waitUntil(
    caches.keys()
        ...
        return Promise.all(
          cacheNames.map((cacheName) => {
            ...
            if (!validCaches.includes(cacheName)) {
              console.log('🗑️ Service Worker v57: Deleting old cache:', cacheName);
      ...
      .then(() => {
        console.log('✅ Service Worker v57: Activated - Old caches cleared');
      ...
      .then((clients) => {
        console.log('📢 Service Worker v57: Notifying clients to refresh');
...
self.addEventListener('message', (event) => {
  ...
  switch (event.data.type) {
    case 'SKIP_WAITING':
      ...
      break;
    case 'GET_VERSION':
      event.ports[0]?.postMessage({ version: 'v57', cache: CACHE_NAME });
      break;
    case 'CLEAR_CACHE':
...
console.log('✅ Service Worker v57: Loaded - app restored with correct JS files');
