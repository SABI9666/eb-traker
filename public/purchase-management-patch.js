...
function render() {
    const main = document.getElementById('mainContent');
    main.innerHTML = `<div class="pm"><div class="pm-hero"><span class="pm-tag">${role === 'purchase' ? 'Purchase portal' : (role === 'coo' ? 'COO · Approvals' : 'Director · Approvals')}</span><h2>Purchase Management</h2><p>Manage purchase records, supporting documents and activity history.</p><div class="pm-actions" style="margin-top:1rem">${role !== 'purchase' ? '<button class="pm-btn" id="pmBack">← Procurement</button>' : ''}<button class="pm-btn" id="pmRefresh">Refresh</button><button class="pm-btn" id="pmExport">Export register</button>${role === 'purchase' ? '<button class="pm-btn primary" id="pmNew">+ New purchase</button>' : ''}</div></div><div class="pm-kpis">${[['Total purchases',rows.length],['Open',rows.filter(r=>r.stage!=='closed').length],['Awaiting approval',rows.filter(r=>['pending','draft'].includes(r.approval?.status || 'draft')).length],['Delivery',rows.filter(r=>r.stage==='delivery').length],['Payment',rows.filter(r=>r.stage==='payment').length],['Closed',rows.filter(r=>r.stage==='closed').length]].map(x=>`<div class="pm-kpi"><b>${x[1]}</b><span>${x[0]}</span></div>`).join('')}</div><div class="pm-card"><div class="pm-tools"><input id="pmSearch" aria-label="Search purchases" placeholder="Search project, item, vendor or reference" value="${esc(query)}"><select id="pmFilter" aria-label="Filter by stage"><option value="">All stages</option>${STAGES.map(s=>`<option value="${s[0]}" ${filter===s[0]?'selected':''}>${s[1]}</option>`).join('')}</select><select id="pmApprovalFilter" aria-label="Filter approval"><option value="">All approval states</option>${['draft','pending','approved','rejected'].map(a=>`<option value="${a}" ${approvalFilter===a?'selected':''}>${a}</option>`).join('')}</select></div><div id="pmRows"></div></div><p class="pm-note">New entries, edits, deletion requests and each document upload require COO or Director approval. Approved details remain in place while changes are reviewed. Payment stages track payments made outside this system.</p></div>`;
...
function operation(r) {
    const change = r.pendingChange;
    if (!change) return r.approval?.status === 'draft' ? 'Initial draft' : '';
    return ({initial:'Initial purchase', edit:'Edit / process change', delete:'Delete purchase', document:'Document: ' + (change.documentType || '')})[change.kind] || '';
}
function rowActions(r) {
    const button = (mode, title, primary = false) => `<button class="pm-btn ${primary?'primary':''}" data-purchase="${esc(r.id)}" data-mode="${mode}">${title}</button>`;
    let html = button('view','View');
    if (role === 'purchase') {
        if (r.approval?.status === 'pending') return html + button('view','View pending request');
        if (r.stage !== 'closed') {
            html += button('edit','Edit');
            if (r.approval?.status === 'approved') html += button('quotation','Add quote',true) + button('po','Add P.O.') + button('other','Add document / process');
            else html += button('view','Submit for approval',true);
        }
        html += button('delete','Request delete');
    } else if (['coo','director'].includes(role) && ['pending','draft'].includes(r.approval?.status || 'draft')) {
        html += button('approve','Review & approve',true) + button('reject','Review & reject');
    }
    return html;
}
function changePreview(r) {
    const c = r.pendingChange;
    if (!c) return '';
    const changes = c.data ? Object.keys(c.data).filter(k => c.data[k] !== r[k]).map(k => [k,r[k],c.data[k]]) : [];
    if (c.stage && c.stage !== r.stage) changes.push(['stage',label(r.stage),label(c.stage)]);
    return `<section class="pm-panel"><h3>Awaiting review: ${esc(operation(r))}</h3><p>${esc(c.note || 'Initial purchase approval')}</p>${c.filename?`<p>Document: ${esc(c.filename)} — download below to review.</p>`:''}${c.kind==='delete'?'<p>Approval removes this purchase from the active register. Its documents and history are retained.</p>':''}${changes.length?`<div class="pm-table-wrap"><table class="pm-table" style="min-width:0"><thead><tr><th>Field</th><th>Current</th><th>Proposed</th></tr></thead><tbody>${changes.map(([k,b,a])=>`<tr><td>${esc(k)}</td><td>${esc(b)}</td><td>${esc(a)}</td></tr>`).join('')}</tbody></table></div>`:''}</section>`;
}
function table() {
    const items = visible();
    document.getElementById('pmRows').innerHTML = items.length ? `<div class="pm-table-wrap"><table class="pm-table"><thead><tr><th>Purchase / project</th><th>Item / quantity</th><th>Vendor</th><th>Value</th><th>Stage / approval</th><th>Required by</th><th>Updated</th><th style="min-width:260px">Actions</th></tr></thead><tbody>${items.map(r=>`<tr><td><b>${esc(r.reference || r.id.slice(0,8))}</b><br>${esc(r.project)}</td><td>${esc(r.item)}<br><small>${esc(r.qty)}</small>${r.priority==='urgent'?'<span class="pm-urgent">URGENT</span>':''}</td><td>${esc(r.vendor || 'Not selected')}</td><td>${esc(money(r))}</td><td><span class="pm-pill">${esc(label(r.stage))}</span><br><small>${esc(r.approval?.status || 'draft')}</small><br><small>${esc(operation(r))}</small></td><td>${esc(r.requiredBy || '—')}</td><td>${esc(date(r.updatedAt))}</td><td><div class="pm-actions">${rowActions(r)}</div></td></tr>`).join('')}</tbody></table></div>` : '<p style="padding:2rem;color:#64748b">No purchases found. '+(role==='purchase'?'Create a purchase to get started.':'Purchase staff entries will appear here.')+'</p>';
    document.querySelectorAll('[data-purchase]').forEach(b=>b.onclick=()=>open(b.dataset.purchase,b.dataset.mode));
}
...
async function open(id, mode = 'view') {
    ...
    try {
        ...
        const r = result.data, approval = r.approval || { status: 'draft' }, editable = role === 'purchase' && !r.deletedAt && r.stage !== 'closed' && approval.status !== 'pending' && (!id || mode === 'edit');
        const canUpload = role === 'purchase' && !r.deletedAt && r.stage !== 'closed' && approval.status === 'approved';
        const detail = modal.querySelector('#pmDetail');
        detail.innerHTML = `${changePreview(r)}${r.lastDecision ? `<p class="pm-note">Last decision: ${esc(r.lastDecision.kind)} · ${esc(r.lastDecision.status)} · ${esc(r.lastDecision.note || "")}</p>` : ""}<section class="pm-panel"><h3>Approval: ${esc(approval.status)}</h3>${approval.decidedBy ? `<p>${esc(approval.decidedBy.name)} · ${esc(date(approval.decidedAt))}</p><p>${esc(approval.note || '')}</p>` : ''}${id && role === 'purchase' && r.stage !== 'closed' ? (approval.status === 'pending' ? '<button class="pm-btn" data-action="withdraw">Withdraw pending request</button>' : approval.status !== 'approved' ? '<button class="pm-btn primary" data-action="submit">Submit saved purchase for approval</button>' : '<p>Approved. Use the row actions to request edits or add documents. Each operation is sent for approval.</p>') : ''}${id && ['coo','director'].includes(role) && ['pending','draft'].includes(approval.status) ? '<label for="pmDecisionNote">Decision note (required for rejection)</label><textarea id="pmDecisionNote" maxlength="2000" rows="3" style="display:block;width:100%;box-sizing:border-box;margin:0.5rem 0"></textarea><div class="pm-actions"><button class="pm-btn primary" data-action="approved">Approve request</button><button class="pm-btn" data-action="rejected">Reject request</button></div>' : ''}<p id="pmDecisionStatus" role="status"></p></section>${id && mode === 'delete' && role === 'purchase' && approval.status !== 'pending' && !r.deletedAt ? '<section class="pm-panel"><h3>Request deletion</h3><p>The entry stays in the register until COO or Director approves deletion.</p><label for="pmDeleteNote">Reason for deletion</label><textarea id="pmDeleteNote" maxlength="2000" rows="3" style="display:block;width:100%"></textarea><button class="pm-btn" data-action="delete-request">Send deletion request</button></section>' : ''}<form id="pmForm"><fieldset ${editable?'':'disabled'} style="border:0;padding:0;margin:0"><div class="pm-form">${field('project','Project',r.project,'text',true)}${field('reference','RFQ / PO reference',r.reference)}${field('item','Item / scope',r.item,'text',true)}${field('qty','Quantity and unit',r.qty,'text',true)}${field('vendor','Vendor',r.vendor)}${field('amount','Budget / order value',r.amount,'number',true)}<div class="pm-field"><label for="pm-currency">Currency</label><select name="currency" id="pm-currency">${['INR','USD','CAD','AUD','GBP','EUR'].map(c=>`<option ${r.currency===c?'selected':''}>${c}</option>`).join('')}</select></div>${field('requiredBy','Required by',r.requiredBy,'date')}<div class="pm-field"><label for="pm-priority">Priority</label><select name="priority" id="pm-priority">${['normal','urgent'].map(c=>`<option ${r.priority===c?'selected':''}>${c}</option>`).join('')}</select></div><div class="pm-field"><label for="pm-stage">Stage</label><select name="stage" id="pm-stage" ${id?'':'disabled'}>${STAGES.map(s=>`<option value="${s[0]}" ${r.stage===s[0]?'selected':''} ${(s[0]==='approval' || (STAGES.indexOf(s)>=4 && approval.status!=='approved'))?'disabled':''}>${s[1]}</option>`).join('')}</select></div>${field('paymentReference','Payment / closure reference',r.paymentReference)}<div class="pm-field full"><label for="pm-specification">Technical requirements / details</label><textarea id="pm-specification" name="specification" maxlength="4000" rows="3">${esc(r.specification)}</textarea></div>${id?'<div class="pm-field full"><label for="pm-note">Reason for edit / process change (required)</label><textarea id="pm-note" name="note" maxlength="2000" rows="2"></textarea></div>':''}</div>${editable?'<button class="pm-btn primary" type="submit">'+(id?'Submit changes for approval':'Create & submit for approval')+'</button>':''}</fieldset><p id="pmSaveStatus" role="status"></p></form>${id?`<h3>Documents</h3><p>PDF, PNG or JPEG · up to 10 MB each</p><div id="pmDocuments">${result.documents.length?result.documents.map(d=>`<div class="pm-event"><span>📄</span><div><button type="button" class="pm-btn" data-doc="${esc(d.id)}">${esc(d.filename)}</button><span>${esc(d.type)} · ${esc(d.status || 'legacy upload')} · ${esc(d.actor.name)} · ${esc(date(d.at))}</span></div></div>`).join(''):'No documents uploaded.'}</div>${canUpload?`<form id="pmUpload" class="pm-tools"><select name="type" aria-label="Document category">${TYPES.map(t=>`<option>${t}</option>`).join('')}</select><input type="file" name="file" accept=".pdf,.png,.jpg,.jpeg" required aria-label="Choose document"><button class="pm-btn primary">Upload & submit for approval</button></form>`:''}<p id="pmUploadStatus" role="status"></p><h3>Activity history</h3>${result.activities.map(a=>`<div class="pm-event"><span class="pm-dot">•</span><div><b>${esc(a.actor.name)} · ${esc(a.action)}</b><span>${esc(date(a.at))} · ${esc(a.actor.role)}</span><p style="font-size:.8rem;white-space:pre-wrap">${esc(typeof a.details==='string'?a.details:label(a.details.from)+' → '+label(a.details.to)+(a.details.note?'\n'+a.details.note:'')+(a.details.changed||[]).map(c=>'\n'+c.field+': '+c.before+' → '+c.after).join(''))}</p></div></div>`).join('')}`:'<p>Create and submit the purchase first. Once approved, add quotes, P.O. and other documents using the actions on the same row.</p>'}`;
        ...
        detail.querySelectorAll('[data-action]').forEach(button => button.onclick = async () => {
            ...
            const note = (action === 'delete-request' ? detail.querySelector('#pmDeleteNote')?.value : detail.querySelector('#pmDecisionNote')?.value) || '';
            if (action === 'delete-request' && !note.trim()) return message(detail.querySelector('#pmDecisionStatus'), 'Enter a reason for deletion.');
            ...
            try {
                await request('/'+id+'/'+(decision?'decision':action), { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ version:r.version, ...(decision?{decision:action,note}:{note}) }) });
                close(); await load(); if (rows.some(row => row.id === id)) await open(id);
        ...
        detail.querySelector('#pmForm').onsubmit = async e => {
            ...
            button.disabled=true; message(detail.querySelector('#pmSaveStatus'),'Submitting for approval…');
        ...
        if (uploader && ['quotation','po','other'].includes(mode)) { uploader.elements.type.value = mode; uploader.elements.file.focus(); }
        if (mode === 'approve' || mode === 'reject') detail.querySelector('#pmDecisionNote')?.focus();
        if (mode === 'delete') detail.querySelector('#pmDeleteNote')?.focus();
        if(uploader) uploader.onsubmit=async e=>{
            ...
            try { await request('/'+id+'/documents',{method:'POST',body:(() => { const form = new FormData(uploader); form.append('version',r.version); return form; })()}); close(); await load(); if (rows.some(row => row.id === id)) await open(id); }
