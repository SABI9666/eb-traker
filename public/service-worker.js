...
// Version: 6.0.2 - Cache Version 52 (Quote Recent Entries fix + BDM analytics email match)
// ============================================

const CACHE_NAME = 'ebtracker-v52';
const STATIC_CACHE = 'ebtracker-static-v52';
const DYNAMIC_CACHE = 'ebtracker-dynamic-v52';
...
self.addEventListener('install', (event) => {
  console.log('🔧 Service Worker v52: Installing...');
  ...
  event.waitUntil(
    Promise.all([
      ...
      caches.open(STATIC_CACHE)
        .then((cache) => {
          console.log('📦 Service Worker v52: Caching static assets');
          return cache.addAll(STATIC_ASSETS);
        }),
      // Cache external scripts (Firebase SDK including Storage)
      caches.open(DYNAMIC_CACHE)
        .then((cache) => {
          console.log('📦 Service Worker v52: Caching Firebase SDK');
          return Promise.all(
    ...
    .then(() => {
      console.log('✅ Service Worker v52: All assets cached');
      return self.skipWaiting();
    })
    .catch((error) => {
      console.error('❌ Service Worker v52: Cache failed', error);
    })
  );
});
...
self.addEventListener('activate', (event) => {
  console.log('🚀 Service Worker v52: Activating...');
  ...
  event.waitUntil(
    caches.keys()
        ...
        return Promise.all(
          cacheNames.map((cacheName) => {
            ...
            if (!validCaches.includes(cacheName)) {
              console.log('🗑️ Service Worker v52: Deleting old cache:', cacheName);
      ...
      .then(() => {
        console.log('✅ Service Worker v52: Activated - Old caches cleared');
      ...
      .then((clients) => {
        console.log('📢 Service Worker v52: Notifying clients to refresh');
...
self.addEventListener('message', (event) => {
  ...
  switch (event.data.type) {
    case 'SKIP_WAITING':
      ...
      break;
    case 'GET_VERSION':
      event.ports[0]?.postMessage({ version: 'v52', cache: CACHE_NAME });
      break;
    case 'CLEAR_CACHE':
...
console.log('✅ Service Worker v52: Loaded - app restored with correct JS files');
